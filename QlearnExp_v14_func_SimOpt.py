...
def prep_QMatrix(num_episodes, lenOfEpisode, warmstartoff, distribution, initial_budget):
    ...
    QFactorGridPrev = load_object("summary_RollMC_norm_rest.pkl")
    pdb.set_trace()
    ...
    for ingre in range(0,len(QFactorGridPrev)*lenOfEpisode):
        ...
        if distribution=="uniform": currentprice=rng2.uniform(low=0.9, high=1.3) #uniform
        elif distribution=="normal":currentprice=rng2.normal(loc=0, scale=0.1) #normal
        else: currentprice=rng2.weibull(0.5)#weibull
    ...
    total_infeas=0
    for episode in range(len(QFactorGridPrev),num_episodes+1):
        ...
        for step in range(1,lenOfEpisode+1):
            ...
            [temp_costTOgo,temp_nutriTOgo,temp_wasteTOgo,MMpolicy_action,num_infeas]=opt.deterministic_equival(step, xfixes, Prices_dict , Recipe_dict, NutritionalValue, num_packsizeoptions, lenOfEpisode, unused)                    
            #pdb.set_trace()
        ...
        if episode % 100 == 0:
            ...
            file_name="QFactorGrid_adv_"+str(episode)+".pkl"
