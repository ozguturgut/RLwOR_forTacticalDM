...
for ingre in range(1,num_ingredients+1):
    min_required[ingre]=10e10
    for menu in range(1, num_menuoptions+1):
        min_required[ingre]=min(int(min_required[ingre]),int(Recipes[menu][ingre-1])) 
...
def prep_QMatrix(num_episodes, lenOfEpisode, warmstartoff, distribution, initial_budget):
    ...
    #QFactorGrid=load_object("QFactorGrid_adv_reds460.pkl")
    ############################################# Algorithm loop #############################################
    Total_days=0
    Total_wasted=0
    Total_spent=0
    ...
    for episode in range(0,num_episodes+1):
        ...
        for step in range(1,lenOfEpisode+1):
            ...
            file_name="QFactorGrid_adv_reds_norm.pkl"
        ...
        if episode % 100 == 0:
            ...
            print("NumOfStates:", len(QFactorGrid)," NumOfPopularStates:", len(keys_above_threshold))
            file_name="QFactorGrid_adv_reds_norm"+str(episode)+".pkl"
            save_object(QFactorGrid, file_name)
...
def simulateQLearn(num_episodes, QFactorGrid, lenOfEpisode, scarcitypricecoeff, scarce_ingredients, distribution, rng, initial_budget):
    global num_MCepisodes, min_required
